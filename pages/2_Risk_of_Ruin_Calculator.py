...
with left_col:
    starting_balance = st.number_input("Starting Balance ($)", min_value=1000.0, value=100000.0, step=1000.0, key="starting_balance")
    daily_drawdown_pct = st.number_input("Daily Drawdown Limit (%)", min_value=0.1, max_value=20.0, value=5.0, step=0.1, key="daily_drawdown_pct")
    overall_drawdown_pct = st.number_input("Overall Drawdown Limit (%)", min_value=0.1, max_value=30.0, value=10.0, step=0.1, key="overall_drawdown_pct")

with right_col:
    win_rate_pct = st.number_input("Full Win Rate (%)", min_value=0.0, max_value=100.0, value=50.0, step=0.01, format="%.2f", key="win_rate_pct")
    partial_win_rate_pct = st.number_input("Partial Win Rate (%)", min_value=0.0, max_value=100.0, value=0.0, step=0.01, format="%.2f", key="partial_win_rate_pct", help="Trades that close in profit but below your full target R.")
    partial_win_r = st.number_input("Partial Win Avg R", min_value=0.01, max_value=10.0, value=0.5, step=0.01, format="%.2f", key="partial_win_r", help="Average R earned on partial win trades.")
    breakeven_rate_pct = st.number_input("Breakeven Rate (%)", min_value=0.0, max_value=100.0, value=0.0, step=0.01, format="%.2f", key="breakeven_rate_pct")
    reward_risk = st.number_input("Full Win Avg R", min_value=0.1, max_value=10.0, value=1.5, step=0.01, format="%.2f", key="reward_risk", help="Average R earned on full win trades.")
    risk_per_trade_pct = st.number_input("Risk Per Trade (% of balance)", min_value=0.1, max_value=5.0, value=1.0, step=0.1, key="risk_per_trade_pct")
    trades_per_day = st.number_input("Trades Per Day", min_value=1, max_value=20, value=3, step=1, key="trades_per_day")
...
ev = (win_rate_pct / 100.0) * float(reward_risk) + (partial_win_rate_pct / 100.0) * float(partial_win_r) - (loss_rate_pct / 100.0) * 1.0
...
with c1:
    target_phase_1_pct = st.number_input("Phase 1 Target Profit (%)", min_value=0.1, max_value=100.0, value=8.0, step=0.1, key="target_phase_1_pct")

if challenge_type in ("2-Phase Challenge", "3-Phase Challenge"):
    with c2:
        target_phase_2_pct = st.number_input("Phase 2 Target Profit (%)", min_value=0.1, max_value=100.0, value=5.0, step=0.1, key="target_phase_2_pct")
else:
    target_phase_2_pct = 0.0

if challenge_type == "3-Phase Challenge":
    with c2b:
        target_phase_3_pct = st.number_input("Phase 3 Target Profit (%)", min_value=0.1, max_value=100.0, value=5.0, step=0.1, key="target_phase_3_pct")
else:
    target_phase_3_pct = 0.0

with c3:
    simulation_runs = st.slider("Simulation Runs", min_value=100, max_value=10000, value=2000, step=100, key="simulation_runs")

with c4:
    max_days_per_phase = st.slider("Max Trading Days (per phase)", min_value=5, max_value=180, value=30, key="max_days_per_phase")
...
def simulate_challenge() -> tuple[bool, bool, bool, bool, float, int | None, int, int]:
    # Phase 1
    ruined, p1_passed, bal1, p1_days, p1_consec = simulate_phase(target_phase_1_pct, use_trailing=use_eod_trailing_stop)
    if ruined:
...
if st.button("Run Simulation", type="primary", key="run_sim_button"):
    ...
    for _ in range(int(simulation_runs)):
        ruined, passed, reached_p2, reached_p3, final_balance, days_to_pass, _total_days, max_consec = simulate_challenge()
